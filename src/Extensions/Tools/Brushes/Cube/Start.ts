...
export const Start = ( uiSession: Editor.IPlayerUISession<ExtensionStorage> ) => {
    ...
    const onExecuteBrush = () => {
        ...
        const from = {
            x: targetBlock.location.x + fromOffset.x,
            y: targetBlock.location.y + fromOffset.y,
            z: targetBlock.location.z + fromOffset.z,
        };
        ...
        const bounds = Server.BlockVolumeUtils.getBoundingBox( { from, to } );
        if (
